...
<main class="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 p-8">
  <header class="flex items-center justify-between">
    ...
    <button class="rounded-xl border border-white/20 px-4 py-2" onclick={() => appState.ui.designMode = !appState.ui.designMode}>
      {appState.ui.designMode ? 'Close design mode' : 'Design mode'}
    </button>
  </header>
